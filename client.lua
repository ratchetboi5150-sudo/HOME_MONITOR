...
-- Spawn purchased vehicle locally
RegisterNetEvent('homecomputer:client:spawnPurchasedVehicle', function(data)
    ...
    QBCore.Functions.SpawnVehicle(data.model, function(veh)
        ...
        QBCore.Functions.Notify("🚗 " .. data.name .. " has been delivered to your location!", "success", 5000)
    end, coords, true)
end)
...
RegisterNUICallback('hireEliteGuard', function(data, cb)
    local p = promise.new()
    QBCore.Functions.TriggerCallback('homecomputer:server:hireEliteGuard', function(res)
        if res and res.success then
            local model = data.model or 's_m_y_blackops_01'
            TriggerEvent('homecomputer:client:spawnEliteGuard', model)
        end
        p:resolve(res)
    end)
    cb(Citizen.Await(p))
        ...
        CreateThread(function()
            ...
            if LoadModel('speedo') and LoadModel('s_m_m_ups_01') then
                ...
                if LoadModel('hei_prop_heist_box') then
                    ...
                    local loopTimeout = 600 -- ~10 minutes max
                    while not delivered and loopTimeout > 0 do
                        local sleep = 500
...
RegisterNetEvent('homecomputer:client:spawnEliteGuard', function(modelName)
    local model = modelName or 's_m_y_blackops_01'
    if model == "g_m_y_mexgoon_01-3" then
        local mexgoon_models = {"g_m_y_mexgoon_01", "g_m_y_mexgoon_02", "g_m_y_mexgoon_03"}
        model = mexgoon_models[math.random(1, #mexgoon_models)]
    end
    ...
    if LoadModel(model) then
        local playerPed = PlayerPedId()
        local pCoords = GetEntityCoords(playerPed)
        local guard = CreatePed(4, GetHashKey(model), pCoords.x + 2.0, pCoords.y + 2.0, pCoords.z, 0.0, true, false)
...
RegisterNetEvent('homecomputer:client:startSilkStreetContract', function(contractId)
    ...
    CreateThread(function()
        ...
        if LoadModel(cData.propModel) then
            ...
            local secured = false
            local propTimeout = 600
            while not secured and propTimeout > 0 do
                local sleep = 1000
                propTimeout = propTimeout - 1
                if not DoesEntityExist(propObj) then break end
        ...
        while not activeBMPickedUp do
            ...
            if dist < 100.0 then
                ...
                if not DoesEntityExist(activeBMPed) then
                    RequestModel(hash)
                    local to = 5000
                    while not HasModelLoaded(hash) and to > 0 do Wait(10); to = to - 10 end
...
local function SpawnJobVehicle(modelName, coords, heading)
    ...
    RequestModel(model)
    local to = 5000
    while not HasModelLoaded(model) and to > 0 do Wait(10); to = to - 10 end
...
local function GrabPackage()
    ...
    local to = 3000
    while not HasAnimDictLoaded("anim@heists@box_carry@") and to > 0 do Wait(10); to = to - 10 end
    ...
    RequestModel(hash)
    local mTo = 5000
    while not HasModelLoaded(hash) and mTo > 0 do Wait(10); mTo = mTo - 10 end
    if HasModelLoaded(hash) then
        local coords = GetEntityCoords(ped)
...
local function DeliverPackage()
    ...
    local to = 3000
    while not HasAnimDictLoaded("mp_am_hold_up") and to > 0 do Wait(10); to = to - 10 end
    if HasAnimDictLoaded("mp_am_hold_up") then
