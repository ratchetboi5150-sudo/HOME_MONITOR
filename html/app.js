...
function selectWallpaper(i) {
  ...
  // Update grid selection immediately
  document.querySelectorAll('.wp-option').forEach((el,j) => el.classList.toggle('selected', j===i));
  // Notify user
  showAppNotification("Wallpaper Changed", "Desktop wallpaper updated.");
}
...
function initLiveTV() {
  ...
  nuiFetch('getTvChannels', {}).then(channels => {
    ...
    tvChannels = [
      ...
      { id: 5, name: "DriveClub PS4 Movies", category: "Movies", url: "https://www.youtube.com/playlist?list=PLhxX4dIBUqkJTg2ZTjQMEdQay-4QtPaAC", logo: "🎮" },
      { id: 6, name: "Forza Horizon 3 Movies", category: "Movies", url: "https://www.youtube.com/playlist?list=PLhxX4dIBUqkIcmCZvUMogUsS-FwaCZ961", logo: "🎮" },
      { id: 7, name: "Free No Copyright Music", category: "Music", url: "https://www.youtube.com/playlist?list=PLhxX4dIBUqkLnksbmpTOUZO8aKFTJFkal", logo: "🎵" },
      { id: 8, name: "Youtube", category: "Entertainment", url: "https://www.youtube.com/channel/UCinwKytiCCIwZse12kVcYlg", logo: "▶️" }
    ];
...
function playTvChannel(ch) {
  ...
  if (ytId) {
    // Render iframe for youtube (YouTube iframe required)
    ...
    // Use embedded mato_tv player for HLS streams
    openMatoPlayer(url, 'video', 50);
  } else {
    // Fallback: standard HTML5 video tag, audio, or iframe
    const isDirectVideo = url.endsWith('.mp4') || url.endsWith('.webm') || url.endsWith('.ogg');
    const isAudio = url.endsWith('.mp3') || url.endsWith('.wav') || url.endsWith('.aac');
    const isImage = url.endsWith('.jpg') || url.endsWith('.jpeg') || url.endsWith('.png') || url.endsWith('.gif');

    if (isDirectVideo) {
      // Use mato_tv for direct video files to keep player consistent
      openMatoPlayer(url, 'video', 50);
    } else if (isAudio) {
      openMatoPlayer(url, 'audio', 50);
    } else if (isImage) {
      openMatoPlayer(url, 'image', 50);
    } else {
      // Unknown/website: general iframe fallback
      const iframe = document.createElement('iframe');
      iframe.className = 'livetv-iframe-tag';
      iframe.src = url;
      iframe.allowFullscreen = true;
...
function closeTvPlayer() {
  ...
  const container = document.getElementById('livetv-player-container');
  if (container) {
    // If mato iframe is present, send stop message first
    const matoIframe = container.querySelector('iframe');
    if (matoIframe && matoIframe.src && matoIframe.src.includes('mato_tv/tv_screen.html')) {
      try {
        matoIframe.contentWindow.postMessage({ action: 'stop' }, '*');
      } catch (e) { console.error('Failed to post stop to mato iframe', e); }
    }
...
function openMatoPlayer(url, type='video', volume=50) {
  const container = document.getElementById('livetv-player-container');
  if (!container) return;
  container.innerHTML = '';
  const iframe = document.createElement('iframe');
  iframe.className = 'livetv-iframe-tag';
  iframe.src = 'mato_tv/tv_screen.html';
  iframe.style.width = '100%';
  iframe.style.height = '100%';
  iframe.onload = function() {
    try {
      // Primary play message (includes desired volume). tv_screen will unmute if volume > 0.
      iframe.contentWindow.postMessage({ action: 'play', type: type, url: url, volume: volume }, '*');
      // Send follow-up volume messages after short delays to cover Hls.js attachTiming or race conditions.
      setTimeout(() => {
        try { iframe.contentWindow.postMessage({ action: 'volume', volume: volume }, '*'); } catch (e) {}
      }, 500);
      setTimeout(() => {
        try { iframe.contentWindow.postMessage({ action: 'volume', volume: volume }, '*'); } catch (e) {}
      }, 1500);
    } catch (e) { console.error('Mato player postMessage failed', e); }
  };
  container.appendChild(iframe);
}

function openMatoPlayerForSelected() {
  let ch = tvChannels.find(c => c.id === tvSelectedChannelId);
  if (!ch) {
    ch = tvChannels[0];
    if (!ch) {
      showTvPlayerError('No channel selected or available to open in external player.');
      return;
    }
  }
  const url = (ch.url || '').trim();
  if (!url) {
    showTvPlayerError('Selected channel has no URL.');
    return;
  }
  openMatoPlayer(url, 'video', 50);
}
...
const SilkProductCatalog = [
    // ── Drugs ──
    { id: 'wet_weed', name: 'Moist Weed', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'coke', name: 'Cocaine', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'coca_leaf', name: 'Cocaine leaves', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'cannabis', name: 'Cannabis', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'marijuana', name: 'Marijuana', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'poppyresin', name: 'Poppy resin', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'heroin', name: 'Heroin', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'lsa', name: 'LSA', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'lsd', name: 'LSD', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: 'meth', name: 'Meth', price: 500, category: 'contraband', subcategory: 'drugs' },
    { id: "joint", name: "Weed Joint", price: 200, category: "contraband", subcategory: "drugs" },
    { id: "weed_whitewidow", name: "White Widow 2g", price: 300, category: "contraband", subcategory: "drugs" },
    { id: "weed_skunk", name: "Skunk 2g", price: 300, category: "contraband", subcategory: "drugs" },
    { id: "weed_purplehaze", name: "Purple Haze 2g", price: 350, category: "contraband", subcategory: "drugs" },
    { id: "weed_ogkush", name: "OG Kush 2g", price: 350, category: "contraband", subcategory: "drugs" },
    { id: "weed_amnesia", name: "Amnesia 2g", price: 400, category: "contraband", subcategory: "drugs" },
    { id: "weed_ak47", name: "AK47 2g", price: 400, category: "contraband", subcategory: "drugs" },
    { id: "cokebaggy", name: "Cocaine Baggy", price: 500, category: "contraband", subcategory: "drugs" },
    { id: "crack_baggy", name: "Crack Baggy", price: 600, category: "contraband", subcategory: "drugs" },
    { id: "meth_crystals", name: "Meth Crystals", price: 700, category: "contraband", subcategory: "drugs" },
    // ── Pharma ──
    { id: "oxy", name: "Oxycodone Pill", price: 300, category: "contraband", subcategory: "pharma" },
    { id: "perc_5", name: "Percocet 5mg", price: 100, category: "contraband", subcategory: "pharma" },
    { id: "perc_7_5", name: "Percocet 7.5mg", price: 150, category: "contraband", subcategory: "pharma" },
    { id: "perc_10", name: "Percocet 10mg", price: 200, category: "contraband", subcategory: "pharma" },
    { id: "perc_15", name: "Percocet 15mg", price: 300, category: "contraband", subcategory: "pharma" },
    { id: "perc_30", name: "Percocet 30mg", price: 500, category: "contraband", subcategory: "pharma" },
    { id: "perc_30_bottle", name: "Percocet 30mg Bottle", price: 12000, category: "contraband", subcategory: "pharma" },
    // ── Supplies (chemicals, seeds, grow supplies, packaging) ──
    { id: 'chemicals', name: 'Chemicals', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'hydrochloric_acid', name: 'Hydrochloric Acid', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'sodium_hydroxide', name: 'Sodium Hydroxide', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'sulfuric_acid', name: 'Sulfuric Acid', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'thionyl_chloride', name: 'Thionyl Chloride', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'liquidmix', name: 'Liquid Chem Mix', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'bakingsoda', name: 'Baking Soda', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'chemicalvapor', name: 'Chemical Vapors', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: 'trimming_scissors', name: 'Trimming Scissors', price: 500, category: 'contraband', subcategory: 'supplies' },
    { id: "weed_whitewidow_seed", name: "White Widow Seed", price: 80, category: "contraband", subcategory: "supplies" },
    { id: "weed_skunk_seed", name: "Skunk Seed", price: 80, category: "contraband", subcategory: "supplies" },
    { id: "weed_purplehaze_seed", name: "Purple Haze Seed", price: 90, category: "contraband", subcategory: "supplies" },
    { id: "weed_ogkush_seed", name: "OG Kush Seed", price: 90, category: "contraband", subcategory: "supplies" },
    { id: "weed_amnesia_seed", name: "Amnesia Seed", price: 100, category: "contraband", subcategory: "supplies" },
    { id: "weed_ak47_seed", name: "AK47 Seed", price: 100, category: "contraband", subcategory: "supplies" },
    { id: "rolling_paper", name: "Rolling Papers", price: 15, category: "contraband", subcategory: "supplies" },
    { id: "empty_weed_bag", name: "Empty Baggies", price: 5, category: "contraband", subcategory: "supplies" },
    { id: "weed_nutrition", name: "Plant Fertilizer", price: 150, category: "contraband", subcategory: "supplies" },
    // ── Weapons ──
    { id: "weapon_knife", name: "Combat Knife", price: 150, category: "contraband", subcategory: "weapons" },
    { id: "weapon_molotov", name: "Molotov Cocktail", price: 500, category: "contraband", subcategory: "weapons" },
    { id: "weapon_pistol", name: "9mm Pistol", price: 1500, category: "contraband", subcategory: "weapons" },
    { id: "weapon_microsmg", name: "Micro SMG", price: 5000, category: "contraband", subcategory: "weapons" },
    { id: "weapon_assaultrifle", name: "Assault Rifle", price: 12500, category: "contraband", subcategory: "weapons" }
];

let silkCart = {};
let silkActiveTab = 'contraband';
let silkActiveCategory = 'all';
let silkDutyActive = false;
let silkDealingActive = false;

const SILK_SUBCATEGORIES = [
...
function initSilkStreet() {
  ...
  silkActiveCategory = 'all';
  
  // Reset nav item highlights
  document.querySelectorAll('#win-silkstreet .silkstreet-nav-item').forEach(el => el.classList.remove('active'));
  const firstNav = document.querySelector('#win-silkstreet .silkstreet-nav-item[data-tab="contraband"]');
  if (firstNav) firstNav.classList.add('active');
  
  // Show only the contraband section
  document.querySelectorAll('#win-silkstreet .silk-section').forEach(el => el.classList.add('hidden'));
  const contrabandSec = document.getElementById('silk-contraband-sec');
  if (contrabandSec) contrabandSec.classList.remove('hidden');
  
  const cartCount = document.getElementById('silk-cart-count');
...
function switchSilkTab(tabName, element) {
    ...
    // Close the cart drawer when switching tabs
    toggleSilkCart(false);
    
    // Update nav item active state — use data-tab attribute for reliability
    document.querySelectorAll('#win-silkstreet .silkstreet-nav-item').forEach(el => el.classList.remove('active'));
    const targetNav = element || document.querySelector('#win-silkstreet .silkstreet-nav-item[data-tab="' + tabName + '"]');
    if (targetNav) targetNav.classList.add('active');
    ...
    const sectionMap = {
        contraband: 'silk-contraband-sec',
        contracts:  'silk-contracts-sec',
        security:   'silk-security-sec',
        dealing:    'silk-dealing-sec'
    };
...
function loadSilkCatalog() {
    ...
    filterBar.className = 'silk-filter-bar';
    SILK_SUBCATEGORIES.forEach(cat => {
        const pill = document.createElement('button');
        pill.className = 'silk-filter-pill' + (silkActiveCategory === cat.key ? ' active' : '');
        pill.innerHTML = `<span class="silk-pill-icon">${cat.icon}</span> ${cat.label}`;
        pill.onclick = () => switchSilkCategory(cat.key);
    ...
    grid.className = 'silk-catalog-products';
    
    const filteredProducts = silkActiveCategory === 'all'
        ? SilkProductCatalog
        : SilkProductCatalog.filter(p => p.subcategory === silkActiveCategory);
    
    if (filteredProducts.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'silk-empty-state';
        empty.innerHTML = '<i class="fa-solid fa-box-open"></i><span>No items in this category</span>';
        grid.appendChild(empty);
    } else {
        filteredProducts.forEach(p => {
            const qty = silkCart[p.id] || 0;
            const card = document.createElement('div');
            card.className = 'silk-card' + (qty > 0 ? ' silk-card-in-cart' : '');
            card.innerHTML = `
                <div>
                    <div class="silk-card-subcategory">${p.subcategory}</div>
                    <h4>${p.name}</h4>
                    <div class="silk-price">$${p.price.toLocaleString()}</div>
                </div>
...
function switchSilkCategory(key) {
    silkActiveCategory = key;
    loadSilkCatalog();
}
...
function hireSilkBodyguard() {
    let model = document.getElementById('silk-bodyguard-type').value;
    showAppNotification("Elite Security", "Hiring bodyguard...");
    nuiFetch('hireEliteGuard', { model: model }).then(res => {
        if (res && res.success) {
...
function dismissSilkBodyguards() {
...
// ═══════════════════════════════════════════════════════════════════
//  PREMIUM MOTORS CATALOG  —  60+ vehicles, 9 classes
//  img: GTA community CDN thumbnails
// ═══════════════════════════════════════════════════════════════════
const DirectMotorCatalog = {
    // ── SUPER CLASS ──────────────────────────────────────────────
    adder:       { name:"Truffade Adder",         cat:"super",  cls:"S+",  speed:9, price:1000000,  desc:"The pinnacle of exotic engineering. Zero to top speed in under 2 seconds flat.",              img:"https://wiki.rage.mp/images/thumb/d/d1/Adder.png/360px-Adder.png" },
    agera:       { name:"Pfister Growler",        cat:"super",  cls:"S+",  speed:9, price:2750000,  desc:"Ultra-rare hyper-GT with active aerodynamics and a twin-turbo flat-six engine.",              img:"https://wiki.rage.mp/images/thumb/5/5b/Growler.png/360px-Growler.png" },
    autarch:     { name:"Overflod Autarch",       cat:"super",  cls:"S+",  speed:9, price:1955000,  desc:"Lightweight Swedish supercar. Street-legal track weapon with adaptive suspension.",             img:"https://wiki.rage.mp/images/thumb/0/03/Autarch.png/360px-Autarch.png" },
    bullet:      { name:"Maibatsu Penumbra FF",   cat:"super",  cls:"S",   speed:8, price:950000,   desc:"Front-wheel-drive sports coupe turned all-wheel-drive monster. Surprisingly agile.",           img:"https://wiki.rage.mp/images/thumb/1/1e/Penumbraff.png/360px-Penumbraff.png" },
    cheetah:     { name:"Grotti Cheetah Classic", cat:"super",  cls:"S",   speed:8, price:650000,   desc:"Classic Italian racing pedigree in a timeless wedge body. Old school soul, modern power.",     img:"https://wiki.rage.mp/images/thumb/d/d9/Cheetahclassic.png/360px-Cheetahclassic.png" },
    cyclone:     { name:"Principe Cyclone",       cat:"super",  cls:"S+",  speed:9, price:1890000,  desc:"All-electric hypercar with instant torque vectoring. Silent but devastatingly fast.",          img:"https://wiki.rage.mp/images/thumb/e/eb/Cyclone.png/360px-Cyclone.png" },
    deveste8:    { name:"Overflod Entity XXR",    cat:"super",  cls:"S+",  speed:10,price:3550000,  desc:"The fastest production car in San Andreas. Carbon fibre monocoque. Brutal aero.",              img:"https://wiki.rage.mp/images/thumb/9/95/EntityXXR.png/360px-EntityXXR.png" },
    emerus:      { name:"Progen Emerus",          cat:"super",  cls:"S+",  speed:9, price:2750000,  desc:"Mid-engine, rear-wheel-drive British track weapon. Carbon everywhere.",                        img:"https://wiki.rage.mp/images/thumb/d/dc/Emerus.png/360px-Emerus.png" },
    entityxf:    { name:"Overflod Entity XF",     cat:"super",  cls:"S",   speed:8, price:795000,   desc:"Scandinavian aerodynamic purity. Wing-assisted top speed over 210mph.",                       img:"https://wiki.rage.mp/images/thumb/e/eb/EntityXF.png/360px-EntityXF.png" },
    fmj:         { name:"Pegassi Furia",          cat:"super",  cls:"S+",  speed:9, price:1850000,  desc:"Italian masterpiece with rear-biased AWD and a howling naturally aspirated V12.",             img:"https://wiki.rage.mp/images/thumb/1/18/Furia.png/360px-Furia.png" },
    osiris:      { name:"Pegassi Osiris",         cat:"super",  cls:"S",   speed:8, price:1950000,  desc:"Long, low and menacing. Central seating position. Purpose-built for speed.",                  img:"https://wiki.rage.mp/images/thumb/e/e7/Osiris.png/360px-Osiris.png" },
    taipan:      { name:"Grotti X80 Proto",       cat:"super",  cls:"S+",  speed:9, price:2700000,  desc:"Leaked factory prototype that somehow made it to the streets. Twin turbos, active aero.",     img:"https://wiki.rage.mp/images/thumb/e/e5/X80Proto.png/360px-X80Proto.png" },
    t20:         { name:"Progen T20",             cat:"super",  cls:"S+",  speed:9, price:2200000,  desc:"British hypercar with F1-derived aerodynamics. Stick to the road like a magnet.",             img:"https://wiki.rage.mp/images/thumb/3/3a/T20.png/360px-T20.png" },
    tempesta:    { name:"Pegassi Tempesta",        cat:"super",  cls:"S",   speed:8, price:750000,   desc:"Italian mid-engine supercar with sharp handling and classic Pegassi style.",                  img:"https://wiki.rage.mp/images/thumb/f/f1/Tempesta.png/360px-Tempesta.png" },
    tyrant:      { name:"Overflod Tyrant",        cat:"super",  cls:"S+",  speed:9, price:2550000,  desc:"Sweden's answer to every Italian and British hypercar. Brutal straight-line speed.",          img:"https://wiki.rage.mp/images/thumb/2/24/Tyrant.png/360px-Tyrant.png" },
    tyrus:       { name:"Progen Tyrus",           cat:"super",  cls:"S+",  speed:9, price:2550000,  desc:"Track-focused open-wheel extreme with massive rear diffuser and front splitter.",              img:"https://wiki.rage.mp/images/thumb/5/58/Tyrus.png/360px-Tyrus.png" },
    vagner:      { name:"Dewbauchee Vagner",      cat:"super",  cls:"S+",  speed:9, price:1535000,  desc:"German precision at its finest. Mid-mounted turbocharged V8 with magnetic ride.",             img:"https://wiki.rage.mp/images/thumb/6/66/Vagner.png/360px-Vagner.png" },
    visione:     { name:"Grotti Visione",         cat:"super",  cls:"S+",  speed:9, price:2250000,  desc:"Ultra-exclusive Italian V12 with a panoramic glass canopy. Stunning and lethal.",             img:"https://wiki.rage.mp/images/thumb/9/93/Visione.png/360px-Visione.png" },
    voltic2:     { name:"Coil Cyclone II",        cat:"super",  cls:"S+",  speed:9, price:2700000,  desc:"Electric-boosted sequel to the original Voltic. Rocket boost technology on board.",           img:"https://wiki.rage.mp/images/thumb/f/f3/CycloneII.png/360px-CycloneII.png" },
    xa21:        { name:"Ocelot XA-21",           cat:"super",  cls:"S+",  speed:9, price:2375000,  desc:"California dreaming meets German engineering. Naturally aspirated, high-revving V10.",        img:"https://wiki.rage.mp/images/thumb/c/c4/XA-21.png/360px-XA-21.png" },
    zentorno:    { name:"Pegassi Zentorno",       cat:"super",  cls:"S+",  speed:9, price:950000,   desc:"Iconic scissor-door Italian with a raging mid-mounted twin-turbo V12.",                       img:"https://wiki.rage.mp/images/thumb/c/ce/Zentorno.png/360px-Zentorno.png" },

    // ── SPORTS CLASS ─────────────────────────────────────────────
    banshee:     { name:"Bravado Banshee",        cat:"sports", cls:"A",   speed:7, price:126000,   desc:"American muscle meets sports car. Roars through corners like it was born for them.",          img:"https://wiki.rage.mp/images/thumb/0/05/Banshee.png/360px-Banshee.png" },
    carbonizzare:{ name:"Grotti Carbonizzare",    cat:"sports", cls:"A",   speed:7, price:195000,   desc:"Targa-top Italian roadster. The sound of that exhaust is worth the price alone.",             img:"https://wiki.rage.mp/images/thumb/c/c6/Carbonizzare.png/360px-Carbonizzare.png" },
    comet2:      { name:"Pfister Comet",          cat:"sports", cls:"A",   speed:7, price:100000,   desc:"Classic rear-engine Pfister sports car. Raw, analogue driving at its very best.",             img:"https://wiki.rage.mp/images/thumb/e/e2/Comet2.png/360px-Comet2.png" },
    elegy:       { name:"Annis Elegy RH8",        cat:"sports", cls:"A",   speed:7, price:95000,    desc:"Japanese tuner legend. All-wheel drive with a front-biased torque split.",                    img:"https://wiki.rage.mp/images/thumb/5/52/ElegyrH8.png/360px-ElegyrH8.png" },
    elegy2:      { name:"Annis Elegy Retro Custom",cat:"sports",cls:"A",   speed:7, price:904000,   desc:"Fully tuned version of the Elegy with widebody kit and racing internals.",                    img:"https://wiki.rage.mp/images/thumb/1/1f/ElegRet.png/360px-ElegRet.png" },
    flashgt:     { name:"Dewbauchee Specter Custom",cat:"sports",cls:"A",  speed:7, price:695000,   desc:"British GT car with a panoramic roof, perfect for long-haul highway blasting.",               img:"https://wiki.rage.mp/images/thumb/1/18/Specterc.png/360px-Specterc.png" },
    italigtb:    { name:"Grotti Itali GTO",       cat:"sports", cls:"A",   speed:7, price:1965000,  desc:"Modern Italian GT with twin-turbo V8 and an exhaust note you'll never forget.",               img:"https://wiki.rage.mp/images/thumb/e/e1/ItaliGTO.png/360px-ItaliGTO.png" },
    jester3:     { name:"Dinka Jester Classic",   cat:"sports", cls:"A",   speed:7, price:665000,   desc:"Japanese roadster with British chassis philosophy. Brilliant in corners.",                     img:"https://wiki.rage.mp/images/thumb/e/e4/JesterClassic.png/360px-JesterClassic.png" },
    neon:        { name:"Pfister Neon",           cat:"sports", cls:"A",   speed:8, price:1500000,  desc:"German electric sports car. Instant torque, near-silent operation, brutal acceleration.",     img:"https://wiki.rage.mp/images/thumb/3/38/Neon.png/360px-Neon.png" },
    pariah:      { name:"Ocelot Pariah",          cat:"sports", cls:"S",   speed:9, price:1420000,  desc:"The fastest sports car in class. Lightweight rear-drive package with screaming exhaust.",     img:"https://wiki.rage.mp/images/thumb/4/48/Pariah.png/360px-Pariah.png" },
    rapidgt:     { name:"Dewbauchee Rapid GT",    cat:"sports", cls:"A",   speed:7, price:132000,   desc:"British grand tourer with a supercharged V8 and a folding hard top.",                        img:"https://wiki.rage.mp/images/thumb/0/09/RapidGT.png/360px-RapidGT.png" },
    revolter:    { name:"Lampadati Revolter",     cat:"sports", cls:"A",   speed:7, price:395000,   desc:"European luxury sports sedan with a strong V8 and all-wheel drive grip.",                     img:"https://wiki.rage.mp/images/thumb/4/4a/Revolter.png/360px-Revolter.png" },
    ruston:      { name:"Lampadati Ruston",       cat:"sports", cls:"A",   speed:7, price:1650000,  desc:"Classic Italian roadster styling with modern mechanicals. Strikingly beautiful.",             img:"https://wiki.rage.mp/images/thumb/6/63/Ruston.png/360px-Ruston.png" },
    schlagen:    { name:"Ocelot Jugular",         cat:"sports", cls:"A",   speed:8, price:1225000,  desc:"German sport sedan that dominates both road and circuit. Understated and devastating.",        img:"https://wiki.rage.mp/images/thumb/a/a6/Jugular.png/360px-Jugular.png" },
    sultan:      { name:"Annis Sultan RS Classic",cat:"sports", cls:"A",   speed:7, price:795000,   desc:"Legendary tuner platform. All-wheel drive, turbo four, motorsport heritage.",                 img:"https://wiki.rage.mp/images/thumb/f/f5/Sultanrs-classic.png/360px-Sultanrs-classic.png" },
    sultanrs:    { name:"Annis Sultan RS",        cat:"sports", cls:"A",   speed:7, price:795000,   desc:"All-wheel drive Japanese rally car for the street. Handles anything.",                        img:"https://wiki.rage.mp/images/thumb/f/f2/Sultanrs.png/360px-Sultanrs.png" },

    // ── MUSCLE CLASS ─────────────────────────────────────────────
    buccaneer:   { name:"Albany Buccaneer",       cat:"muscle", cls:"B",   speed:5, price:32000,    desc:"1960s American luxury. V8 rumble, chrome trim, and enough torque to rip a road apart.",       img:"https://wiki.rage.mp/images/thumb/0/02/Buccaneer.png/360px-Buccaneer.png" },
    coquette:    { name:"Invetero Coquette",      cat:"muscle", cls:"B",   speed:6, price:150000,   desc:"American sports car with classic Corvette DNA. V8 naturally aspirated and proud of it.",      img:"https://wiki.rage.mp/images/thumb/5/58/Coquette.png/360px-Coquette.png" },
    dukes:       { name:"Imponte Dukes",          cat:"muscle", cls:"B",   speed:5, price:63000,    desc:"Dodge Charger-inspired muscle machine. Classic wide-body style with modern big-block power.",  img:"https://wiki.rage.mp/images/thumb/4/44/Dukes.png/360px-Dukes.png" },
    gauntlet:    { name:"Bravado Gauntlet Classic Custom",cat:"muscle",cls:"B",speed:6,price:645000,desc:"Wide-body pony car with all-out drag setup. Quarter-mile terror.",                            img:"https://wiki.rage.mp/images/thumb/f/fc/GauntletClassicCustom.png/360px-GauntletClassicCustom.png" },
    greenwood:   { name:"Declasse Mamba",         cat:"muscle", cls:"B",   speed:5, price:915000,   desc:"Shelby Cobra-inspired roadster. No roof, no mercy. Raw V8 right behind the nose.",            img:"https://wiki.rage.mp/images/thumb/1/12/Mamba.png/360px-Mamba.png" },
    impaler:     { name:"Declasse Impaler",       cat:"muscle", cls:"B",   speed:5, price:875000,   desc:"Race-prepped 1960s lowrider muscle car. Slammed, cammed, and ready for war.",                 img:"https://wiki.rage.mp/images/thumb/e/e6/Impaler.png/360px-Impaler.png" },
    sabregt:     { name:"Declasse Sabre Turbo",   cat:"muscle", cls:"B",   speed:6, price:490000,   desc:"Turbocharged muscle car with wide flares and a proper exhaust note.",                         img:"https://wiki.rage.mp/images/thumb/d/d4/Sabreturbo.png/360px-Sabreturbo.png" },
    stalion:     { name:"Bravado Gauntlet Hellfire",cat:"muscle",cls:"A",  speed:7, price:745000,   desc:"700+ HP supercharged HEMI pony car. More wheel spin than most dare to handle.",               img:"https://wiki.rage.mp/images/thumb/8/84/Hellfire.png/360px-Hellfire.png" },

    // ── SUV / OFF-ROAD ────────────────────────────────────────────
    baller4:     { name:"Albany Cavalcade II",    cat:"suv",    cls:"C",   speed:4, price:73000,    desc:"American luxury SUV. Cavernous interior, full-size luxury, park-anywhere size.",               img:"https://wiki.rage.mp/images/thumb/7/73/Cavalcade2.png/360px-Cavalcade2.png" },
    brawler:     { name:"BF Injection",           cat:"off-road",cls:"C",  speed:4, price:130000,   desc:"Sand-rail style beach buggy. Light, nimble and absolutely at home in the dirt.",              img:"https://wiki.rage.mp/images/thumb/e/e0/Bfinjection.png/360px-Bfinjection.png" },
    brawler2:    { name:"Canis Mesa (Merryweather)",cat:"off-road",cls:"C",speed:4, price:285000,   desc:"Merryweather-spec Mesa with roll cage and combat suspension package.",                        img:"https://wiki.rage.mp/images/thumb/7/72/MesaMerryweather.png/360px-MesaMerryweather.png" },
    everon:      { name:"Gallivanter Baller LE",  cat:"suv",    cls:"C",   speed:4, price:90000,    desc:"Long-wheelbase luxury SUV. Plush interior, smooth ride, presence on the road.",               img:"https://wiki.rage.mp/images/thumb/4/4d/BallerLE.png/360px-BallerLE.png" },
    granger:     { name:"Declasse Granger",       cat:"suv",    cls:"C",   speed:4, price:35000,    desc:"Full-size American body-on-frame SUV. Common, reliable, and impossible to ignore.",            img:"https://wiki.rage.mp/images/thumb/e/e2/Granger.png/360px-Granger.png" },
    insurgent:   { name:"HVY Insurgent Pick-Up",  cat:"off-road",cls:"B",  speed:5, price:675000,   desc:"Military-grade mine-resistant vehicle. Can survive almost anything San Andreas throws.",       img:"https://wiki.rage.mp/images/thumb/d/db/InsurgentPickUp.png/360px-InsurgentPickUp.png" },
    kamacho:     { name:"Canis Kamacho",          cat:"off-road",cls:"C",  speed:4, price:345000,   desc:"Hardcore off-roader with live axles and a lifted suspension. Goes everywhere.",               img:"https://wiki.rage.mp/images/thumb/5/5b/Kamacho.png/360px-Kamacho.png" },
    landstalker: { name:"Canis Mesa",             cat:"suv",    cls:"C",   speed:4, price:35000,    desc:"Compact SUV with off-road capability built in. Gets the job done anywhere.",                  img:"https://wiki.rage.mp/images/thumb/8/8e/Mesa.png/360px-Mesa.png" },
    seminole2:   { name:"Canis Seminole Frontier",cat:"off-road",cls:"C",  speed:4, price:745000,   desc:"Lifted crew-cab pickup with a supercharger hood scoop. Serious off-road kit.",               img:"https://wiki.rage.mp/images/thumb/d/dc/Seminalefrontier.png/360px-Seminalefrontier.png" },
    toros:       { name:"Canis Toros",            cat:"suv",    cls:"C",   speed:5, price:595000,   desc:"Performance SUV with AWD and adaptive air suspension. Luxury meets brute.",                   img:"https://wiki.rage.mp/images/thumb/c/cb/Toros.png/360px-Toros.png" },

    // ── MOTORCYCLES ──────────────────────────────────────────────
    bati2:       { name:"Dinka Akuma",            cat:"moto",   cls:"A",   speed:8, price:9000,     desc:"Japanese naked bike. Lightweight, explosive power delivery. Urban hunter.",                   img:"https://wiki.rage.mp/images/thumb/4/47/Akuma.png/360px-Akuma.png" },
    carbonrs:    { name:"Shitzu Carbon RS",       cat:"moto",   cls:"A",   speed:7, price:35000,    desc:"Japanese middleweight sportsbike. Perfect balance of handling and speed.",                     img:"https://wiki.rage.mp/images/thumb/5/58/Carbonrs.png/360px-Carbonrs.png" },
    chimera:     { name:"LCC Chimera",            cat:"moto",   cls:"B",   speed:6, price:1450000,  desc:"Bagger-style custom cruiser with custom rigid frame. Rolling art piece.",                     img:"https://wiki.rage.mp/images/thumb/3/30/Chimera.png/360px-Chimera.png" },
    deathbike:   { name:"Western Deathbike",      cat:"moto",   cls:"S",   speed:9, price:1269000,  desc:"Apocalypse-ready armoured motorbike. Blades, rockets, and terrifying speed.",                img:"https://wiki.rage.mp/images/thumb/5/5b/Deathbike.png/360px-Deathbike.png" },
    diabolus2:   { name:"Western Reever",         cat:"moto",   cls:"A",   speed:8, price:1900000,  desc:"Cutting-edge electric superbike from Western. Ultra-quiet, ultra-fast.",                      img:"https://wiki.rage.mp/images/thumb/d/dd/Reever.png/360px-Reever.png" },
    gargoyle:    { name:"Western Gargoyle",       cat:"moto",   cls:"A",   speed:7, price:120000,   desc:"Custom American chopper. Extended forks, loud V-twin, and pure intimidation.",                img:"https://wiki.rage.mp/images/thumb/8/83/Gargoyle.png/360px-Gargoyle.png" },
    hakuchou:    { name:"Shitzu Hakuchou Drag",   cat:"moto",   cls:"S",   speed:10,price:976000,   desc:"Factory drag-spec superbike. Longest wheelbase. Lowest, fastest, most dangerous.",            img:"https://wiki.rage.mp/images/thumb/2/2d/Hakuchou-drag.png/360px-Hakuchou-drag.png" },
    oppressor2:  { name:"Pegassi Oppressor Mk II",cat:"moto",   cls:"S",   speed:9, price:3890250,  desc:"Rocket-boosted hover bike with missiles. The apex predator of San Andreas skies.",            img:"https://wiki.rage.mp/images/thumb/6/68/OppressorMKII.png/360px-OppressorMKII.png" },
    pcj:         { name:"Dinka Sugoi",            cat:"moto",   cls:"A",   speed:8, price:1595000,  desc:"Electric Japanese superbike with active battery cooling system on display.",                  img:"https://wiki.rage.mp/images/thumb/6/6c/Sugoi.png/360px-Sugoi.png" },
    shotaro:     { name:"Nagasaki Shotaro",       cat:"moto",   cls:"S",   speed:9, price:2225000,  desc:"TRON-inspired lightcycle design. Extreme speed with stunning visual presence.",               img:"https://wiki.rage.mp/images/thumb/4/4d/Shotaro.png/360px-Shotaro.png" },

    // ── SEDANS & COMPACTS ─────────────────────────────────────────
    fugitive:    { name:"Albany Primo Custom",    cat:"sedan",  cls:"D",   speed:3, price:105000,   desc:"Sleeper sedan with a performance engine tucked under stock bodywork.",                        img:"https://wiki.rage.mp/images/thumb/e/e3/PrimoCustom.png/360px-PrimoCustom.png" },
    jackal:      { name:"Lampadati Furore GT",    cat:"sedan",  cls:"C",   speed:5, price:490000,   desc:"Italian grand tourer with a naturally aspirated V6 and a gorgeous silhouette.",               img:"https://wiki.rage.mp/images/thumb/9/92/Furoregt.png/360px-Furoregt.png" },
    kuruma:      { name:"Karin Kuruma (Armoured)",cat:"sedan",  cls:"B",   speed:5, price:698250,   desc:"Armoured sedan with bullet-resistant glass. Safe as a tank, drives like a car.",              img:"https://wiki.rage.mp/images/thumb/7/7d/Kuruma2.png/360px-Kuruma2.png" },
    sentinel3:   { name:"Ocelot Lynx",            cat:"sedan",  cls:"A",   speed:7, price:1080000,  desc:"German sport coupe with a turbocharged inline-six and sport-biased rear drive.",              img:"https://wiki.rage.mp/images/thumb/4/43/Lynx.png/360px-Lynx.png" },
    schafter3:   { name:"Weeny Dynasty",          cat:"sedan",  cls:"C",   speed:4, price:465000,   desc:"Classic Chinese luxury sedan with retro styling. Surprisingly refined inside.",               img:"https://wiki.rage.mp/images/thumb/1/17/Dynasty.png/360px-Dynasty.png" },

    // ── TRUCKS & VANS ─────────────────────────────────────────────
    benson:      { name:"Declasse Hellion",       cat:"truck",  cls:"C",   speed:4, price:525000,   desc:"Sand-spec lifted pickup with a supercharger and off-road tyres. Mean and practical.",         img:"https://wiki.rage.mp/images/thumb/3/33/Hellion.png/360px-Hellion.png" },
    rumpo3:      { name:"Bravado Rat-Truck",      cat:"truck",  cls:"C",   speed:4, price:42500,    desc:"Pre-runner rat-look pickup with a radical stance and off-road suspension kit.",               img:"https://wiki.rage.mp/images/thumb/b/b3/Rat-truck.png/360px-Rat-truck.png" },
    youga3:      { name:"Vapid Caracara 4x4",     cat:"truck",  cls:"C",   speed:4, price:875000,   desc:"American 4x4 pickup with a lifted suspension and baja racing heritage.",                      img:"https://wiki.rage.mp/images/thumb/8/8c/Caracara4x4.png/360px-Caracara4x4.png" },
    dubsta3:     { name:"Vapid FMJ",              cat:"truck",  cls:"A",   speed:7, price:1750000,  desc:"Purpose-built supercar-killer sports car from Vapid. Front-engine, rear-drive, V8.",          img:"https://wiki.rage.mp/images/thumb/e/e4/FMJ.png/360px-FMJ.png" },

    // ── SPECIAL / MILITARY ────────────────────────────────────────
    technical2:  { name:"HVY Nightshark",         cat:"special",cls:"B",   speed:5, price:1245000,  desc:"Armoured luxury SUV with optional heavy machine gun. Bulletproof windows standard.",          img:"https://wiki.rage.mp/images/thumb/a/a5/Nightshark.png/360px-Nightshark.png" },
    halftrack:   { name:"HVY APC",                cat:"special",cls:"B",   speed:4, price:3092250,  desc:"Amphibious armoured personnel carrier with cannon turret. Goes anywhere.",                   img:"https://wiki.rage.mp/images/thumb/f/f3/APC.png/360px-APC.png" },
    squaddie:    { name:"Vapid Caracara",          cat:"special",cls:"C",   speed:5, price:695000,   desc:"Military-influenced off-road pickup. Stripped back and built to last.",                      img:"https://wiki.rage.mp/images/thumb/4/40/Caracara.png/360px-Caracara.png" },
};

// ── Category metadata ────────────────────────────────────────────
const MotorCategories = [
    { id: "all",      label: "All Vehicles",   icon: "fa-th",             color: "#ff9900" },
    { id: "super",    label: "Super",          icon: "fa-rocket",         color: "#ff003c" },
    { id: "sports",   label: "Sports",         icon: "fa-flag-checkered", color: "#ff6600" },
    { id: "muscle",   label: "Muscle",         icon: "fa-fire",           color: "#d97706" },
    { id: "suv",      label: "SUV / Luxury",   icon: "fa-car-side",       color: "#7c3aed" },
    { id: "off-road", label: "Off-Road",       icon: "fa-mountain",       color: "#059669" },
    { id: "moto",     label: "Motorcycles",    icon: "fa-motorcycle",     color: "#0ea5e9" },
    { id: "sedan",    label: "Sedans",         icon: "fa-car",            color: "#6366f1" },
    { id: "truck",    label: "Trucks & Vans",  icon: "fa-truck",          color: "#78716c" },
    { id: "special",  label: "Military / Special", icon: "fa-shield",     color: "#16a34a" },
];

let directCart = {};
let directActiveTab = 'gear';
let motorActiveCat = 'all';
let motorSearchQuery = '';
let motorSelectedId = null;
...
function loadDirectCatalog() {
    ...
    // Motors section is rendered separately by renderMotorDealership()
    if (directActiveTab === 'motors') {
        renderMotorDealership();
    }
...
// ══ MOTOR DEALERSHIP ENGINE ══════════════════════════════════════

function renderMotorDealership() {
    // Build category pills
    const catBar = document.getElementById('do-motor-cats');
    if (catBar) {
        catBar.innerHTML = '';
        MotorCategories.forEach(cat => {
            const pill = document.createElement('button');
            pill.className = 'do-cat-pill' + (motorActiveCat === cat.id ? ' active' : '');
            pill.style.setProperty('--cat-color', cat.color);
            pill.innerHTML = `<i class="fa-solid ${cat.icon}"></i> ${cat.label}`;
            pill.onclick = () => { motorActiveCat = cat.id; renderMotorGrid(); updateCatPills(); };
...
function updateCatPills() {
    document.querySelectorAll('.do-cat-pill').forEach((pill, i) => {
        pill.classList.toggle('active', MotorCategories[i]?.id === motorActiveCat);
    });
}

function renderMotorGrid() {
    const grid = document.getElementById('do-motors-catalog');
    if (!grid) return;

    const query = (document.getElementById('do-motor-search')?.value || '').toLowerCase();
    motorSearchQuery = query;

    const filtered = Object.entries(DirectMotorCatalog).filter(([id, m]) => {
        const catMatch = motorActiveCat === 'all' || m.cat === motorActiveCat;
        const searchMatch = !query || m.name.toLowerCase().includes(query) || id.toLowerCase().includes(query);
        return catMatch && searchMatch;
    });

    if (filtered.length === 0) {
        grid.innerHTML = `<div class="do-motor-empty"><i class="fa-solid fa-car-burst"></i><p>No vehicles found</p></div>`;
        return;
    }

    grid.innerHTML = '';
    filtered.forEach(([id, m]) => {
        const isInCart = directCart[id] ? true : false;
        const card = document.createElement('div');
        card.className = 'do-veh-card' + (isInCart ? ' in-cart' : '') + (motorSelectedId === id ? ' selected' : '');
        card.dataset.id = id;
        card.onclick = () => openVehicleDetail(id);
        const clsColors = { 'S+': '#ff003c', 'S': '#ff4060', 'A': '#ff9900', 'B': '#22c55e', 'C': '#60a5fa', 'D': '#94a3b8' };
        const clsColor = clsColors[m.cls] || '#ff9900';
        const speedBars = Array.from({length: 10}, (_, i) =>
            `<span class="do-spd-bar${i < m.speed ? ' filled' : ''}"></span>`
        ).join('');
        card.innerHTML = `
            <div class="do-veh-img" style="background-image:url('${m.img}')">
                <span class="do-veh-cls-badge" style="background:${clsColor}20;color:${clsColor};border-color:${clsColor}40">${m.cls}</span>
                ${isInCart ? '<span class="do-veh-cart-badge"><i class="fa-solid fa-check"></i> In Cart</span>' : ''}
            </div>
            <div class="do-veh-body">
                <div class="do-veh-name">${m.name}</div>
                <div class="do-veh-speed-row">
                    <span class="do-spd-label">TOP SPEED</span>
                    <div class="do-spd-bars">${speedBars}</div>
                </div>
                <div class="do-veh-price">$${m.price.toLocaleString()}</div>
            </div>
        `;
        grid.appendChild(card);
    });
...
function openVehicleDetail(id) {
    motorSelectedId = id;
    const m = DirectMotorCatalog[id];
    if (!m) return;

    document.querySelectorAll('.do-veh-card').forEach(c => c.classList.remove('selected'));
    const card = document.querySelector(`.do-veh-card[data-id="${id}"]`);
    if (card) card.classList.add('selected');

    const panel = document.getElementById('do-veh-detail');
    if (!panel) return;

    const clsColors = { 'S+': '#ff003c', 'S': '#ff4060', 'A': '#ff9900', 'B': '#22c55e', 'C': '#60a5fa', 'D': '#94a3b8' };
    const clsColor = clsColors[m.cls] || '#ff9900';
    const catLabel = MotorCategories.find(c => c.id === m.cat)?.label || m.cat;
    const speedBars = Array.from({length: 10}, (_, i) =>
        `<span class="do-spd-bar large${i < m.speed ? ' filled' : ''}"></span>`
    ).join('');
    const canAfford = (playerData.bank || 0) >= m.price;

    panel.innerHTML = `
        <div class="do-detail-img" style="background-image:url('${m.img}')">
            <span class="do-veh-cls-badge lg" style="background:${clsColor}20;color:${clsColor};border-color:${clsColor}40">${m.cls} Class</span>
        </div>
        <div class="do-detail-body">
            <div class="do-detail-name">${m.name}</div>
            <div class="do-detail-cat"><i class="fa-solid fa-tag"></i> ${catLabel}</div>
            <p class="do-detail-desc">${m.desc}</p>
            <div class="do-detail-stats">
                <div class="do-stat-row">
                    <span class="do-stat-label"><i class="fa-solid fa-gauge-high"></i> Top Speed</span>
                    <div class="do-spd-bars">${speedBars}</div>
                </div>
            </div>
            <div class="do-detail-price-row">
                <span class="do-detail-price">$${m.price.toLocaleString()}</span>
                <span class="do-detail-balance ${canAfford ? 'ok' : 'low'}">
                    <i class="fa-solid fa-university"></i> Bank: $${(playerData.bank || 0).toLocaleString()}
                </span>
            </div>
            <div class="do-detail-delivery">
                <span class="do-dlv-label">DELIVERY</span>
                <label class="do-dlv-opt">
                    <input type="radio" name="motor-delivery" value="depot" checked>
                    <span><i class="fa-solid fa-warehouse"></i> Register to Depot Garage</span>
                </label>
                <label class="do-dlv-opt">
                    <input type="radio" name="motor-delivery" value="spawn">
                    <span><i class="fa-solid fa-location-dot"></i> Spawn at My Location</span>
                </label>
            </div>
            <button class="do-purchase-btn ${!canAfford ? 'disabled' : ''}" onclick="purchaseVehicle('${id}')" ${!canAfford ? 'disabled' : ''}>
                <i class="fa-solid fa-key"></i> ${canAfford ? 'Purchase Vehicle' : 'Insufficient Funds'}
            </button>
        </div>
    `;
...
function purchaseVehicle(id) {
    ...
    const deliveryEl = document.querySelector('input[name="motor-delivery"]:checked');
    const delivery = deliveryEl ? deliveryEl.value : 'depot';

    const btn = document.querySelector('.do-purchase-btn');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Processing...';
    }

    nuiFetch('chooseDirectDelivery', {
        items: [{ id: id, qty: 1, type: 'motor', delivery: delivery }],
        delivery: delivery,
        totalCost: m.price
    }).then(res => {
        if (res && res.success) {
            showAppNotification('Premium Motors', `🚗 ${m.name} purchased! ${delivery === 'depot' ? 'Check your depot garage.' : 'Vehicle spawned nearby.'}`);
            playerData.bank = Math.max(0, (playerData.bank || 0) - m.price);
            updateHomeSystemInfo();
            // Mark in-cart visually
            directCart[id] = { qty: 1, type: 'motor' };
            if (btn) {
                btn.innerHTML = '<i class="fa-solid fa-check"></i> Purchase Complete!';
            ...
            showAppNotification('Payment Failed', (res && res.error) || 'Insufficient bank funds!');
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="fa-solid fa-key"></i> Purchase Vehicle';
            }
        }
    });
}
...
window.switchSilkCategory = switchSilkCategory;
...
window.hireSilkBodyguard = hireSilkBodyguard;
window.dismissSilkBodyguards = dismissSilkBodyguards;
