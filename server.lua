    ...
    CreateThread(function()
        ...
        if count == 0 then
            local defaults = {
                ...
                { name = "DriveClub PS4 Movies", category = "Movies", url = "https://www.youtube.com/playlist?list=PLhxX4dIBUqkJTg2ZTjQMEdQay-4QtPaAC", logo = "🎮" },
                { name = "Forza Horizon 3 Movies", category = "Movies", url = "https://www.youtube.com/playlist?list=PLhxX4dIBUqkIcmCZvUMogUsS-FwaCZ961", logo = "🎮" },
                { name = "Free No Copyright Music", category = "Music", url = "https://www.youtube.com/playlist?list=PLhxX4dIBUqkLnksbmpTOUZO8aKFTJFkal", logo = "🎵" },
                { name = "Youtube", category = "Entertainment", url = "https://www.youtube.com/channel/UCinwKytiCCIwZse12kVcYlg", logo = "▶️" }
            }
...
RegisterNetEvent('homecomputer:server:sendRadioMessage', function(frequency, message)
    ...
    if #channel.chat > 20 then
        table.remove(channel.chat, 1)
    end
...
local function GetUniquePlate()
    ...
    while IsPlateTaken(plate) and maxAttempts > 0 do
...
QBCore.Functions.CreateCallback('homecomputer:server:payDirectOrder', function(source, cb, items, totalCost)
    ...
    -- Use RemoveBankMoney helper (tries qb-banking export first, then standard QBCore)
    if RemoveBankMoney(Player, cost, "direct-order-vehicle-purchase") then
        for _, item in ipairs(items) do
            if item.type == "motor" then
                local plate = GetUniquePlate()
                local delivery = item.delivery or 'depot'
                MySQL.insert("INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", {
                    ...
                    '{}',
                    plate,
                    delivery == 'spawn' and 'outside' or 'depot',
                    delivery == 'spawn' and 0 or 1
                }, function(insertId)
                    if delivery == 'spawn' then
                        -- Send spawn request to the client
                        TriggerClientEvent('homecomputer:client:spawnPurchasedVehicle', src, {
                            model  = item.id,
                            plate  = plate,
                            name   = item.id:upper()
                        })
                    else
                        TriggerClientEvent('QBCore:Notify', src, "🚗 " .. item.id:upper() .. " registered to depot! Plate: " .. plate, "success", 8000)
                    end
                end)
            end
        end
...
RegisterNetEvent('homecomputer:server:claimContractReward', function(contractId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end

    -- Validate payout amount server-side instead of trusting client
    local configContract = Config.SilkContractLocations[contractId]
    if not configContract then return end
    
    local rewardAmt = configContract.payout
    if not rewardAmt or rewardAmt <= 0 then return end

    Player.Functions.AddMoney('cash', rewardAmt, "silkstreet-contract-reward")
    TriggerClientEvent('QBCore:Notify', src, "Contract complete. You received $" .. rewardAmt, "success")
end)
...
QBCore.Functions.CreateCallback('homecomputer:server:scanSPHeists', function(source, cb)
    ...
    local cmd = ('dir "%s" /B 2>nul'):format(folderPath)
    ...
    if handle then
        ...
        cmd = ('ls "%s" 2>/dev/null'):format(resourcePath .. "/sp_heists")
        handle = io.popen(cmd)
        if handle then
            for file in handle:lines() do
                if file and file ~= "" then
...
QBCore.Functions.CreateCallback('homecomputer:server:scanBlackMarket', function(source, cb)
    ...
    if handle then
        ...
        cmd = ('ls "%s" 2>/dev/null'):format(resourcePath .. "/black_market")
        handle = io.popen(cmd)
        if handle then
            for file in handle:lines() do
                if file == "TheBlackMarket.dll" then dllExists = true end
...
RegisterNetEvent('homecomputer:server:claimDriverPay', function(tier)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if Player.PlayerData.job.name == "driver" then
        ...
        local amount = payoutMap[tier] or 500

        Player.Functions.AddMoney('bank', amount, "delivery-driver-pay")
        TriggerClientEvent('QBCore:Notify', src, "Delivery completed! $" .. amount .. " added to bank.", "success")
    else
